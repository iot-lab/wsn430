...
uint16_t phy_send_frame(uint8_t frame[], uint16_t length)
{
  ...
  cc1100_gdo0_register_callback(tx_ok);
  ...
  if (tx_length > 63)
  {
    ...
    cc1100_cfg_fifo_thr(8);
    
    cc1100_cfg_gdo2(CC1100_GDOx_TX_FIFO);
    cc1100_gdo2_int_set_falling_edge();
    cc1100_gdo2_int_clear();
    cc1100_gdo2_int_enable();
    cc1100_gdo2_register_callback(tx_refill);
    
    tx_index  = 63;
    
    cc1100_fifo_put((uint8_t*)(&tx_length), 1);
    cc1100_fifo_put(tx_buffer, 63);
  }
  else
  {
...
uint16_t phy_start_rx(void)
{
  ...
  cc1100_cfg_fifo_thr(RADIO_FIFOTHR_SETING);
    
  cc1100_cfg_gdo2(CC1100_GDOx_RX_FIFO);
  cc1100_gdo2_int_set_rising_edge();
  cc1100_gdo2_int_clear();
  cc1100_gdo2_int_enable();
  cc1100_gdo2_register_callback(rx_unfill);
...
uint16_t phy_cca(void)
{
  ...
  switch (radio_status)
  {
    case RADIO_STATUS_IDLE:
      ...
      pktstatus = cc1100_status_pktstatus();
      // go back to idle
      cc1100_cmd_idle();
      break;
...
static uint16_t rx_ok(void) {
  ...
  if (rx_index == 0)
  {
    cc1100_fifo_get((uint8_t*)&rx_length, 1);
  }
  
  // read the remaining bytes from the FIFO
  len = cc1100_status_rxbytes();
  ...
  cc1100_fifo_get(rx_buffer+rx_index, len);
...
static uint16_t rx_unfill(void)
{
  ...
  if (cc1100_status_rxbytes() < RADIO_RX_THR)
  {
    return 1;
  }
  ...
  cc1100_fifo_get(rx_buffer+rx_index, RADIO_RX_THR);
...
static uint16_t tx_refill(void)
{
  ...
  if (cc1100_status_txbytes() > RADIO_TX_THR)
  {
    return 1;
  }
  ...
  cc1100_fifo_put(tx_buffer+tx_index, len);
  tx_index += len;
