...
\chapter{Program example}

In this chapter, we're going to build an entire application from \textit{almost} scratch, using FreeRTOS for the OS on the nodes. Here are the main elements of the application:
\begin{description}
...
the \textbf{mac} task, implementing a simple MAC layer allowing the node to communicate must be very reactive to incomming and outgoing communications, therefore it will have the highest priority.
